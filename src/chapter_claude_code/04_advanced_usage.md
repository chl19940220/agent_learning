...
```
Claude Code
    ├── MCP: github      → 读取 PR、创建 Issue、触发 CI 流水线
    ├── MCP: postgres    → 查询生产数据库、分析慢查询、生成 ER 图
    ├── MCP: jira        → 创建任务、更新状态、关联代码提交
    ├── MCP: slack       → 发送通知、读取频道消息、@相关负责人
    └── MCP: your-api    → 调用你的内部服务、查询部署状态
```
...
```
事件类型        触发时机              退出码含义              典型用途
─────────────────────────────────────────────────────────────────────
PreToolUse     工具执行前            0 = 允许继续            安全审计、阻断危险命令
                                    2 = 阻断工具调用         参数校验、注入额外上下文

PostToolUse    工具执行后            0 = 正常处理            代码格式化、自动运行测试
                                    （无法回滚）             写入操作日志、触发通知

SessionStart   会话创建/恢复时       0 = 继续会话            注入动态上下文
                                                            加载项目状态、环境检查

SessionEnd     会话正常结束时        0 = 继续退出            保存会话摘要
                                                            触发下游任务、清理临时文件

Stop           Claude 停止响应时     0 = 允许停止            任务完成通知
                                    2 = 阻止停止            强制执行最终验证步骤

PreCompact     上下文压缩前          0 = 继续压缩            将关键状态写入 CLAUDE.md
                                                            保存不可丢失的决策记录
```
...
```
工作流是否跨多个项目复用？
    ├── 是 → 考虑 Skills（~/.claude-internal/skills/）
    └── 否 → 考虑 /commands（.claude/commands/）

工作流是否包含多步骤或需要引用附属文件？
    ├── 是 → Skills（支持完整目录结构）
    └── 否 → Prompt Template（在 CLAUDE.md 中写一段说明即可）

是否需要附带脚本（.sh/.py）？
    ├── 是 → Skills（可以在 SKILL.md 中引用并调用脚本）
    └── 否 → /commands（单文件即可）
```
...
```
主 Agent（Team Lead / 协调者）
    │
    ├──[并行]──→ 子 Agent A（独立上下文）：审查前端变更
    │                └── 完成后将结果返回给主 Agent
    │
    ├──[并行]──→ 子 Agent B（独立上下文）：审查后端变更
    │                └── 完成后将结果返回给主 Agent
    │
    └──[并行]──→ 子 Agent C（独立上下文）：审查数据库迁移
                     └── 完成后将结果返回给主 Agent
    
主 Agent 汇总所有结果，生成最终报告
```
...
```
# 对一个改动了 20 个文件的 PR 进行审查

# 低效方式（串行）：
Claude 逐一审查 20 个文件 → 上下文越来越长 → 后期审查质量下降

# 高效方式（Sub-agents）：
主 Agent：将 20 个文件按模块分为 5 组
    ├── 子 Agent 1：审查 auth 模块（4个文件）
    ├── 子 Agent 2：审查 payment 模块（5个文件）
    ├── 子 Agent 3：审查 API 层（4个文件）
    ├── 子 Agent 4：审查数据层（4个文件）
    └── 子 Agent 5：审查测试文件（3个文件）
    → 每个子 Agent 在干净的上下文中工作，审查质量更高
    → 5个并行 vs 串行，时间缩短约 60%
```
...
主 Agent：
    ├── 子 Agent 1：深度调研 Saga 模式（实现原理、优缺点、适用场景）
    ├── 子 Agent 2：深度调研 TCC 模式（实现原理、优缺点、适用场景）
    └── 子 Agent 3：深度调研 2PC 协议（实现原理、优缺点、适用场景）
→ 三个 Agent 并行调研，主 Agent 汇总对比，结合项目特点给出选型建议
```
...
上下文利用率
    ...
    40 ~ 70%： ⚡ microcompact（轻量压缩，自动触发）
               保留：最近 N 轮对话 + 当前任务描述 + 重要决策
               丢弃：中间过程、已完成任务的详情、重复信息
               用户感知：几乎无感，对话继续
    
    70 ~ 85%： ⚡⚡ autocompact（深度压缩，自动触发）
               等价于执行 /compact
               保留：当前任务核心状态 + 关键文件内容摘要
               丢弃：大部分对话历史
               用户感知：Claude 会提示"已压缩上下文"
...
```
长期记忆层（跨会话持久）：
    ~/.claude/CLAUDE.md          ← 全局永久记忆（所有项目始终加载）
    [项目]/CLAUDE.md             ← 项目级永久记忆（在该项目始终加载）
