...
.claude/hooks/     ✅            API Key
.claude/commands/  ✅            个人快捷键配置
```
...
```
System Prompt（静态区）  ← 跨会话复用，缓存命中率 90%+
    ↓
用户消息（动态区）
  ├── CLAUDE.md 内容    ← 内容不变时可被缓存
  ├── 当前任务描述      ← 每次变化，不缓存
  └── 对话历史          ← 逐渐增长，缓存失效
```
...
任务类型                          推荐模型
──────────────────────────────────────────────
日常代码修改、Bug 修复            Claude Sonnet（默认）
复杂架构设计、多文件重构          Claude Opus（/ultraplan）
简单格式化、注释补全              Claude Haiku（最经济）
CI/CD 自动化审查                 Claude Sonnet（平衡）
```
...
```
日常开发：Cursor（IDE 内补全，高效）
复杂任务：Claude Code（终端，Agent 模式）
CI/CD：Claude Code Headless（自动化）
代码审查：Claude Code（结构化报告）
```
