...
```mermaid
flowchart LR
    subgraph 传统补全范式["传统补全范式（Copilot / Cursor）"]
        direction LR
        A1["用户写代码"] --> B1["AI 预测下一行/函数"] --> C1["用户接受/拒绝"]
    end
    subgraph Agent范式["Agent 范式（Claude Code）"]
        direction LR
        A2["用户描述目标"] --> B2["AI 制定计划"] --> C2["AI 调用工具执行"] --> D2["AI 验证结果"] --> E2["循环直到完成"]
    end
```
...
```mermaid
flowchart TD
    A["用户指令"] --> B["理解意图（明确任务目标）"]
    B --> C["探索上下文（读代码、读文档、运行命令）"]
    C --> D["制定执行计划"]
    D --> E["调用工具"]
    E --> F["观察结果"]
    F --> G["调整计划"]
