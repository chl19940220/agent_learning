...
用户在终端输入消息并按下 Enter
   ...
   （拼装 System Prompt：静态区 + 动态区，详见 15.3 节）
         │
         ▼
   processUserInput()
...
```
Claude 输出工具调用请求（JSON 格式）
        │
        ▼
Tool Registry 查找对应工具
        │
        ▼
权限系统裁决（详见 15.3 节）
   ├── Allow  → 执行工具
